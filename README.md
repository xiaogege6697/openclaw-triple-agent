...
| Linux | ✅ 完全支持 | 服务器/VPS 均可 |
| Windows | ⚠️ 需要 WSL | 通过 WSL2 或 Git Bash 运行 |
...
- Node.js >= 22
- Git
- **OpenClaw 2026.3.13**（install.sh 自动安装固定版本，**请勿使用最新版**）

> ⚠️ **版本锁定**：本配置包基于 OpenClaw `2026.3.13` 开发测试，新版可能存在不兼容。install.sh 会自动锁定版本，如已安装其他版本会提示切换。
...
- **Windows 用户**：需先安装 [WSL2](https://learn.microsoft.com/zh-cn/windows/wsl/install)，然后在 WSL 终端中运行 install.sh
